e\documentclass[fontsize=12pt]{scrartcl} 
		...
		como una combinaci\'on lineal de ellos.
		   ...
		   se tiene que $\left( \frac{a}{(a,b)}, \frac{b}{(a,b)} \right ) = 1$. \\
                  ...
                  \end{equation*}
                    ...
                    Ya sabemos que  $\left( \frac{\abs {a} } { (\abs{a},\abs{b})  }, \frac{\abs{b}}{(\abs{a},\abs{b})} \right) = \prod_{i=1}^{k} p_i ^{\phi_i} $ donde $\phi_i =$ mín$\{\alpha_i-\gamma_i, \beta_i-\gamma_i\}$. \\
                    Veremos que, por la definición de $\gamma_i$ debe pasar que $\alpha_i-\gamma_i = 0$ ó $\beta_i-\gamma_i = 0$. \\
                    Hay 3 casos: \\
                    \begin{enumerate}
                    \item $\alpha_i = \beta_i$: \\
                      En este caso,  $\gamma_i$ = mín$\{\alpha_i, \beta_i\} = \alpha_i = \beta_i$. Por lo tanto, $\alpha_i-\gamma_i = 0$ y $\beta_i-\gamma_i = 0$.
                      Con esto, ya que  $\phi_i =$ mín$\{\alpha_i-\gamma_i, \beta_i-\gamma_i\} =$  mín $\{0, 0\} = 0$.
                    \item $\alpha_i < \beta_i$: \\
                    En este caso, $\gamma_i$= mín$\{\alpha_i, \beta_i\} = \alpha_i$. Por lo tanto, $\alpha_i-\gamma_i = 0$, además $\beta_i-\gamma_i > 0$.
                    Con esto, ya que  $\phi_i =$ mín$\{\alpha_i-\gamma_i, \beta_i-\gamma_i\} =$  mín $\{0, \beta_i-\gamma_i \} = 0$.
                     \item $\alpha_i > \beta_i$: \\
                    En este caso, $\gamma_i$ = mín$\{\alpha_i, \beta_i\} = \beta_i$. Por lo tanto, $\beta_i-\gamma_i = 0$, además $\alpha_i-\gamma_i > 0$.
                    Con esto, ya que  $\phi_i =$ mín$\{\alpha_i-\gamma_i, \beta_i-\gamma_i\} =$  mín $\{ \alpha_i-\gamma_i, 0 \} = 0$.
                    \end{enumerate}

                    Ya que demostramos que $\phi_i =0$ para toda $1 \leq i \leq k$. Entonces: $\left( \frac{\abs {a} } { (\abs{a},\abs{b})  }, \frac{\abs{b}}{(\abs{a},\abs{b})} \right) = \prod_{i=1}^{k} p_i ^{\phi_i} =  \prod_{i=1}^{k} p_i ^{0} = p_1^{0} \cdots p_k^{0}  = 1 \cdots 1 =1$.
